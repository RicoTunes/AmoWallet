...
class _TransactionsPageEnhancedState extends ConsumerState<TransactionsPageEnhanced>
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        tx.type == 'swap' ? 'Swap' : (tx.type == 'sent' ? 'Sent' : 'Received'),
                        style: const TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                        ),
                      ...
                      // Details
                      _buildDetailRow('Date', DateFormat('MMM d, yyyy • HH:mm').format(tx.timestamp)),
                      _buildDetailRow('Network', tx.coin),
                      if (tx.fee != null && tx.fee! > 0) _buildDetailRow('Fee', '${tx.fee!.toStringAsFixed(8)} ${tx.coin.split('-').first}'),
                      // For sent: show "To" with toAddress, For received: show "From" with fromAddress
                      if (tx.type == 'sent' && tx.toAddress != null && tx.toAddress!.isNotEmpty)
                        _buildDetailRow('To', tx.toAddress!, isAddress: true)
                      else if (tx.type == 'sent')
                        _buildDetailRow('To', tx.address, isAddress: true),
                      if (tx.type == 'received' && tx.fromAddress != null && tx.fromAddress!.isNotEmpty)
                        _buildDetailRow('From', tx.fromAddress!, isAddress: true)
                      else if (tx.type == 'received')
                        _buildDetailRow('From', tx.address, isAddress: true),
                      if (tx.txHash != null && tx.txHash!.isNotEmpty)
                        _buildHashRow('Tx Hash', tx.txHash!),
                          ...
                          Expanded(
                            child: ElevatedButton.icon(
                              onPressed: () async {
                                ...
                                final explorerUrl = _getExplorerUrl(tx.coin, hash);
                                if (explorerUrl != null) {
  ...
  String? _getExplorerUrl(String coin, String txHash) {
    final baseCoin = coin.split('-').first.toUpperCase();
    ...
    switch (baseCoin) {
      ...
      case 'USDT':
        // USDT could be on different networks
        if (coin.contains('BEP20')) {
          return 'https://bscscan.com/tx/$txHash';
        } else if (coin.contains('TRC20')) {
          return 'https://tronscan.org/#/transaction/$txHash';
        }
        return 'https://etherscan.io/tx/$txHash'; // Default to ERC20
      case 'SOL':
